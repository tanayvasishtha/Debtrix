...
-- Debts table (matches your TypeScript exactly)
...
CREATE POLICY "Allow demo users for debts" ON public.debts FOR ALL USING (user_id::text LIKE 'demo-%');

CREATE POLICY "Users can manage their own assessment" ON public.debt_assessment FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Allow demo users for assessment" ON public.debt_assessment FOR ALL USING (user_id::text LIKE 'demo-%');

CREATE POLICY "Users can manage their own progress" ON public.progress_tracking FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Allow demo users for progress" ON public.progress_tracking FOR ALL USING (user_id::text LIKE 'demo-%');

CREATE POLICY "Users can manage their own chat history" ON public.chat_history FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Allow demo users for chat" ON public.chat_history FOR ALL USING (user_id::text LIKE 'demo-%');
